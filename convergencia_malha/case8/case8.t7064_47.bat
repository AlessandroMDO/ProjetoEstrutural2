...
set JIDDIR=c:/users/user/desktop/saa205/entregas/projeto/convergencia_malha/case8
set MSC_APP=no
set MSC_AUTH=edu
set MSC_BASE=c:/program files/msc.software/napa_se/20211/nastran
set MSC_DBS=c:/scratch/case8.T7064_47
set MSC_EXE='c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/analysis.exe'
set MSC_ISHELLPATH=c:/users/user/desktop/saa205/entregas/projeto/convergencia_malha/case8
set MSC_ISHELLEXT=
set MSC_JID=C:\Users\User\Desktop\SAA205\ENTREGAS\PROJETO\convergencia_malha\case8\case8.bdf
set MSC_JIDPATH=
set MSC_JIDTYPE=.bdf
set MSC_MEM=527MB
set MSC_OLD=yes
...
set PWD=C:\Users\User\Desktop\SAA205\ENTREGAS\PROJETO\convergencia_malha\case8
set PWD=%PWD:/=\%
cd /d %PWD%
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./case8.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./case8.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=C:\Program Files\MSC.Software\NaPa_SE\20211\Nastran\msc20211\win64i8\nastran.exe case8.bdf prog=edu auth=edu bat=no
...
if exist "case8.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem case8.perf >> %lognt%
if exist "%sdirnt%\case8.T7064_47.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem %sdirnt%\case8.T7064_47.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\case8.T7064_47.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\case8.T7064_47.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\case8.T7064_47.f99"+"%sdirnt%\case8.T7064_47.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\case8.T7064_47.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\case8.T7064_47.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\case8*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\case8.T7064_47.*") do call :delete_job_file "%%f"
...
if exist "case8.perf" del /f "case8.perf"
if "%mksol700%" == "yes" (
   if exist ./case8.sol700.pth del /f ./case8.sol700.pth
)
...
setlocal
set ext=%~x1
type "case8_SN_*.%1" > case8.%1 2>nul
del /f "case8_SN_*.%1" 2>nul:
>nul findstr "^" "case8.%1" || del "case8.%1"
endlocal
goto :eof
@rem
@rem
