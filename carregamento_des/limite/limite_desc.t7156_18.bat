...
set datecmd=%datecmd:/=\%
set exedir=
set jobnt=./limite_desc.T7156_18
set jobnt=%jobnt:/=\%
set lognt=./limite_desc.log
set lognt=%lognt:/=\%
set modedir=
...
set JIDDIR=c:/users/user/desktop/saa205/entregas/projeto/carregamento_des/limite
set MSC_APP=no
set MSC_AUTH=edu
set MSC_BASE=c:/program files/msc.software/napa_se/20211/nastran
set MSC_DBS=c:/scratch/limite_desc.T7156_18
set MSC_EXE='c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/analysis.exe'
set MSC_ISHELLPATH=c:/users/user/desktop/saa205/entregas/projeto/carregamento_des/limite
set MSC_ISHELLEXT=
set MSC_JID=C:\Users\User\Desktop\SAA205\ENTREGAS\PROJETO\carregamento_des\limite\limite_desc.bdf
set MSC_JIDPATH=
set MSC_JIDTYPE=.bdf
set MSC_MEM=527MB
set MSC_OLD=yes
set MSC_OUT=./limite_desc
set MSC_SCR=yes
...
set PWD=C:\Users\User\Desktop\SAA205\ENTREGAS\PROJETO\carregamento_des\limite
set PWD=%PWD:/=\%
cd /d %PWD%
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./limite_desc.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./limite_desc.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=C:\Program Files\MSC.Software\NaPa_SE\20211\Nastran\msc20211\win64i8\nastran.exe limite_desc.bdf prog=edu auth=edu bat=no
...
if exist "limite_desc.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem limite_desc.perf >> %lognt%
if exist "%sdirnt%\limite_desc.T7156_18.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem %sdirnt%\limite_desc.T7156_18.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\limite_desc.T7156_18.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\limite_desc.T7156_18.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\limite_desc.T7156_18.f99"+"%sdirnt%\limite_desc.T7156_18.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\limite_desc.T7156_18.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\limite_desc.T7156_18.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\limite_desc*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\limite_desc.T7156_18.*") do call :delete_job_file "%%f"
...
if exist "limite_desc.perf" del /f "limite_desc.perf"
if "%mksol700%" == "yes" (
   if exist ./limite_desc.sol700.pth del /f ./limite_desc.sol700.pth
)
...
setlocal
set ext=%~x1
type "limite_desc_SN_*.%1" > limite_desc.%1 2>nul
del /f "limite_desc_SN_*.%1" 2>nul:
>nul findstr "^" "limite_desc.%1" || del "limite_desc.%1"
endlocal
goto :eof
@rem
@rem
