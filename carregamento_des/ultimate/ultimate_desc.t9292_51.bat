...
set datecmd=%datecmd:/=\%
set exedir=
set jobnt=./ultimate_desc.T9292_51
set jobnt=%jobnt:/=\%
set lognt=./ultimate_desc.log
set lognt=%lognt:/=\%
set modedir=
...
set JIDDIR=c:/users/user/desktop/saa205/entregas/projeto/carregamento_des/ultimate
set MSC_APP=no
set MSC_AUTH=edu
set MSC_BASE=c:/program files/msc.software/napa_se/20211/nastran
set MSC_DBS=c:/scratch/ultimate_desc.T9292_51
set MSC_EXE='c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/analysis.exe'
set MSC_ISHELLPATH=c:/users/user/desktop/saa205/entregas/projeto/carregamento_des/ultimate
set MSC_ISHELLEXT=
set MSC_JID=C:\Users\User\Desktop\SAA205\ENTREGAS\PROJETO\carregamento_des\ultimate\ultimate_desc.bdf
set MSC_JIDPATH=
set MSC_JIDTYPE=.bdf
set MSC_MEM=527MB
set MSC_OLD=yes
set MSC_OUT=./ultimate_desc
set MSC_SCR=yes
...
set PWD=C:\Users\User\Desktop\SAA205\ENTREGAS\PROJETO\carregamento_des\ultimate
set PWD=%PWD:/=\%
cd /d %PWD%
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./ultimate_desc.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./ultimate_desc.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=C:\Program Files\MSC.Software\NaPa_SE\20211\Nastran\msc20211\win64i8\nastran.exe ultimate_desc.bdf prog=edu auth=edu bat=no
...
if exist "ultimate_desc.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem ultimate_desc.perf >> %lognt%
if exist "%sdirnt%\ultimate_desc.T9292_51.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem %sdirnt%\ultimate_desc.T9292_51.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\ultimate_desc.T9292_51.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\ultimate_desc.T9292_51.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\ultimate_desc.T9292_51.f99"+"%sdirnt%\ultimate_desc.T9292_51.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\ultimate_desc.T9292_51.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\ultimate_desc.T9292_51.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\ultimate_desc*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\ultimate_desc.T9292_51.*") do call :delete_job_file "%%f"
...
if exist "ultimate_desc.perf" del /f "ultimate_desc.perf"
if "%mksol700%" == "yes" (
   if exist ./ultimate_desc.sol700.pth del /f ./ultimate_desc.sol700.pth
)
...
setlocal
set ext=%~x1
type "ultimate_desc_SN_*.%1" > ultimate_desc.%1 2>nul
del /f "ultimate_desc_SN_*.%1" 2>nul:
>nul findstr "^" "ultimate_desc.%1" || del "ultimate_desc.%1"
endlocal
goto :eof
@rem
@rem
